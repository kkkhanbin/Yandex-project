...
from constants.level_editor.level_editor_settings import TITLE, LOWER_MARGIN, \
    SIDE_MARGIN, ROWS, MAX_COLS, TILE_TYPES, TILE_SIZE, RED, GREEN, BLUE, WHITE

from constants.level_editor.level_editor_settings import SCREEN_WIDTH, \
    SCREEN_HEIGHT
...
class Editor(Window):
    def __init__(self) -> None:
        ...
        self.background1_img = pygame.image.load(
            "img/Background/background1.png").convert_alpha()
        self.background2_img = pygame.image.load(
            "img/Background/background2.png").convert_alpha()
        self.background3_img = pygame.image.load(
            "img/Background/background3.png").convert_alpha()
        ...
        self.self.save_button = button.Button(
            SCREEN_WIDTH // 2 + 150, SCREEN_HEIGHT + LOWER_MARGIN - 75,
            save_img, 1)
        self.load_button = button.Button(SCREEN_WIDTH // 2 + 300,
                                         SCREEN_HEIGHT + LOWER_MARGIN - 75,
                                         load_img, 1)
        ...
        for i in range(len(self.images_list)):
            tile_button = button.Button(
                SCREEN_WIDTH + (75 * button_col) + 50, 75 * button_row + 50,
                self.images_list[i], 1)
    ...
    def draw_background(self):
        ...
        for i in range(3):
            screen.blit(self.background3_img, (
                (i * width) - self.scroll * 0.5, 0))
            screen.blit(self.background1_img, (
                (i * width) - self.scroll * 0.7,
                SCREEN_HEIGHT - self.background1_img.get_height() - 150))
            screen.blit(self.background2_img, (
                (i * width) - self.scroll * 0.8,
                SCREEN_HEIGHT - self.background2_img.get_height()))

    # функция для рисовки клеток
    def draw_grid(self):
        # вертикальные линии
        for i in range(MAX_COLS + 1):
            pygame.draw.line(screen, WHITE, (i * TILE_SIZE - self.scroll, 0), (
                i * TILE_SIZE - self.scroll, SCREEN_HEIGHT))
    ...
    def draw_world(self):
        for i, row in enumerate(self.world_data):
            for j, tile in enumerate(row):
                if tile >= 0:
                    screen.blit(self.images_list[tile], (
                        j * TILE_SIZE - self.scroll, i * TILE_SIZE))
    ...
    def run(self):
        while self.get_running():
            ...
            self.draw_grid()
            self.draw_world()
            self.draw_text(f"Уровень: {self.level}", self.font, WHITE, 10,
                           SCREEN_HEIGHT + LOWER_MARGIN - 90)
            self.draw_text("Нажмите вверх или вниз, чтобы изменить уровень",
                           self.font, WHITE, 10,
            ...
            if self.save_button.draw(screen):
                ...
                with open(f"level{self.level}_data.csv", "w", newline="") \
                        as csvfile:
            ...
            if self.load_button.draw(screen):
                self.scroll = 0
                with open(f"level{self.level}_data.csv", newline="") \
                        as csvfile:
            ...
            pygame.draw.rect(
                screen, RED, self.button_list[current_tile].rect, 3)

            # прокрутка карты
            if self.scroll_left is True and self.scroll > 0:
                self.scroll -= 5 * self.scroll_speed
            if self.scroll_right is True and \
               self.scroll < (MAX_COLS * TILE_SIZE) - SCREEN_WIDTH:
                self.scroll += 5 * self.scroll_speed
