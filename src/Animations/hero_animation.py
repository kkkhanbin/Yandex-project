...
def strip_from_sheet():
    ...
    for row in range(rows):
        for col in range(columns):
            rect = pygame.Rect(
                col * img_width, row * img_height, img_width, img_height)
    ...
    for row in range(rows):
        for col in range(columns):
            rect = pygame.Rect(
                col * img_width, row * img_height, img_width, img_height)
            frames.append(
                pygame.transform.flip(sheet.subsurface(rect), True, False))
...
image = pygame.image.load(
    r"Yandex-project-develop\src\data\images\animations\Hero_Move.png").convert(
        )
